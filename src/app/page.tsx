...
export default function Home() {
  ...
  const [localHandsReady, setLocalHandsReady] = useState(false);
  ...
  const [homeSection, setHomeSection] = useState<"menu" | "groups" | "espace">("menu");
    ...
    <main className={`flex h-screen flex-col overflow-hidden text-zinc-100 ${PHR_PAGE_BG}`}>
      ...
      <div className="mx-auto flex min-h-0 w-full max-w-[1520px] flex-1 flex-col px-3 pb-3 pt-2 sm:px-5 sm:pb-4 sm:pt-3">
        <section className={PHR_APP_SHELL}>
          ...
          {viewSpotReview && spotReviewPost && <PhrSpotReviewInfoBar post={spotReviewPost} />}
          ...
          {hands.length > 0 && !viewPublicHome && !viewSpotReview && !viewTournamentReview && (
            <div className={PHR_TOURNAMENT_RAIL}>
              ...
              {selectedTournament !== "ALL" ? (
                <button
                  type="button"
                  onClick={() => openPublishTournament(selectedTournament)}
                  className="shrink-0 rounded-xl border border-violet-500/45 bg-violet-600/25 px-3 py-2 text-xs font-bold text-violet-100 transition hover:border-violet-400/55 hover:bg-violet-600/35"
                >
                  Publier le tournoi
                </button>
              ) : null}
          ...
          {hands.length > 0 && !viewPublicHome && !viewSpotReview && !viewTournamentReview && allInTriggered && (
          ...
          <div className={`${PHR_TABLE_FRAME}${viewSpotReview || viewTournamentReview ? " h-full min-h-0" : ""}`}>
            ...
            <button
              ...
              className={`absolute right-2 z-[45] rounded-full border border-white/12 bg-gradient-to-b from-zinc-700/50 to-zinc-950/90 px-3 py-2 text-xs font-bold text-zinc-100 shadow-[0_4px_20px_rgba(0,0,0,0.4),inset_0_1px_0_rgba(255,255,255,0.08)] backdrop-blur-md transition hover:border-white/20 sm:right-3 sm:px-3.5 sm:py-2.5 sm:text-sm ${
                viewSpotReview ? "bottom-16 sm:bottom-16" : "bottom-2 sm:bottom-3"
              }`}
            >
          ...
          if (viewTournamentReview && tournamentReview) {
            return (
              <div className="flex min-h-0 flex-1 flex-col sm:flex-row">
                ...
                <PhrTournamentReviewPanel
                  tournament={tournamentReview.tournament}
                  hands={tournamentReview.hands}
                  selectedHandDocId={tournamentSelectedHandDocId}
                  onSelectHand={loadTournamentHandByDocId}
                  onBack={closeTournamentReview}
                />
              </div>
            );
          }
